...
export class VectorDbService {
  ...
  async initializeIndex() {
    ...
    try {
      ...
      if (
        ...
        error.message?.includes("IndexAlreadyExists")
      ) {
        console.log(
          `Pinecone index ${this.indexName} already exists, using existing index`
        );
        this.index = this.pinecone.index(this.indexName);
        return this.index;
      }
  ...
  async createChunks(apiIndex, intentMappings, namespace) {
    ...
    for (const endpoint of apiIndex.endpoints) {
      ...
      if (endpoint.businessTags && endpoint.businessTags.length > 0) {
        chunks.push({
          endpointId: endpoint.endpointId,
          chunkType: "tag",
          content: `Tags: ${endpoint.businessTags.join(", ")}. Endpoint: ${
            endpoint.method
      ...
      if (endpoint.examples) {
        if (
          ...
          endpoint.examples.requests.length > 0
        ) {
          endpoint.examples.requests.forEach((req, idx) => {
            chunks.push({
              endpointId: endpoint.endpointId,
              chunkType: "example",
              content: `Request example for ${endpoint.method} ${
                endpoint.path
              }:\n${JSON.stringify(req.body, null, 2)}`,
              metadata: {
                title: `Request example ${idx + 1} for ${endpoint.method} ${
                  endpoint.path
                }`,
        ...
        if (
          endpoint.examples.responses &&
          endpoint.examples.responses.length > 0
        ) {
          endpoint.examples.responses.forEach((resp, idx) => {
            chunks.push({
              endpointId: endpoint.endpointId,
              chunkType: "example",
              content: `Response example (${resp.statusCode}) for ${
                endpoint.method
              } ${endpoint.path}:\n${JSON.stringify(resp.body, null, 2)}`,
              metadata: {
                title: `Response example ${idx + 1} for ${endpoint.method} ${
                  endpoint.path
                }`,
      ...
      if (endpoint.parameters && endpoint.parameters.length > 0) {
        const paramDescriptions = endpoint.parameters
          .map(
            (p) =>
      ...
      for (const intent of intentMappings.intents) {
        ...
        if (intent.subIntents) {
          for (const subIntent of intent.subIntents) {
            ...
            if (
              subIntent.exampleQueries &&
              subIntent.exampleQueries.length > 0
            ) {
    ...
    for (const chunk of chunks) {
      try {
        ...
        const chunkId = `chunk_${apiIndex._id}_${Date.now()}_${Math.random()
          .toString(36)
          .substr(2, 9)}`;
    ...
    if (vectorsToUpsert.length > 0 && !this.useMockMode) {
      try {
        ...
        console.log(
          `Upserted ${vectorsToUpsert.length} vectors to Pinecone namespace: ${namespace}`
        );
      } catch (error) {
      ...
      console.log(
        `🔄 [MOCK] Skipped Pinecone upsert for ${vectorsToUpsert.length} vectors`
      );
    }
  ...
  async generateEmbedding(text) {
    ...
    if (this.useMockMode) {
      console.log(
        `🔄 [MOCK] Generating random embedding for: "${text.substring(
          0,
          50
        )}..."`
      );
      ...
      const mockEmbedding = Array.from(
        { length: this.embeddingDimensions },
        () => Math.random() - 0.5
      );
      console.log(
        `✅ [MOCK] Generated ${mockEmbedding.length}-dimensional vector`
      );
    ...
    try {
      ...
      if (!embedding || embedding.length === 0) {
        console.error(
          "Empty embedding returned from API. Response:",
          JSON.stringify(data)
        );
  ...
  buildSubIntentContent(subIntent, parentIntent) {
    ...
    if (subIntent.description)
      content += `Description: ${subIntent.description}\n`;
  ...
  async searchChunks(query, apiIndexId, namespace, limit = 10) {
    ...
    try {
      ...
      const queryResponse = await this.index
        ...
        .query(queryOptions);
      console.log(
        `Searched namespace: ${namespace}, found ${
      ...
      if (process.env.PINECONE_USE_FILTER !== "true") {
        matches = matches.filter(
          (match) => match.metadata?.apiIndexId === apiIndexId.toString()
        );
      }
