...
export class ProcessingPipeline {
  ...
  async process(
    ...
    try {
      ...
      const apiIndex = await this.createApiIndex(
        apiDocId,
        parsed,
        zohoOrgId,
        namespace,
      ...
      const businessTags = await this.llmService.generateBusinessTags(
        parsed.endpoints
      );
      ...
      const intentMapping = await this.createIntentMapping(
        apiIndex._id,
        intents
      );
      log(
        `Created ${intents.length} intents with ${intents.reduce(
          (sum, i) => sum + (i.subIntents?.length || 0),
          0
        )} sub-intents`
      );
      ...
      const updatedIntentMapping = await IntentMapping.findById(
        intentMapping._id
      );
      ...
      const chunks = await this.vectorDbService.createChunks(
        updatedApiIndex,
        updatedIntentMapping,
        namespace
      );
  ...
  async createApiIndex(
    ...
    const apiIndex = new ApiIndex({
      apiDocId: null,
      zohoOrgId,
      namespace, // Not using ApiDoc model in onboarding flow
  ...
  async createIntentMapping(apiIndexId, intents) {
    ...
    const processedIntents = intents.map((intent, idx) => ({
      ...
      subIntents: (intent.subIntents || []).map((subIntent, subIdx) => ({
        subIntentId:
          subIntent.subIntentId || `sub_${apiIndexId}_${idx}_${subIdx}`,
        name: subIntent.name,
  ...
  async getStatus(apiDocId) {
    ...
    const intentMapping = await IntentMapping.findOne({
      apiIndexId: apiIndex._id,
    });
